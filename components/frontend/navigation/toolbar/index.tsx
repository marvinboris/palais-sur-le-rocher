...
import { XMarkIcon, HomeIcon, IdentificationIcon, Bars3BottomRightIcon, PhoneIcon, WrenchIcon, ShoppingBagIcon, UsersIcon, MegaphoneIcon, NewspaperIcon } from '@heroicons/react/24/outline'
...
const exploreItems = [
    { name: 'A propos', description: "Tout savoir sur le Palais sur le Rocher et la Mission évangélique du secours divin.", href: '/explore/about', icon: HomeIcon },
    { name: 'Staff pastoral', description: "Retrouvez les leaders de notre commission.", href: '/explore/pastoral-staff', icon: UsersIcon },
    { name: 'Objet de la mission', description: "La mission qu'a donnée l'Eternel à notre commission.", href: '/explore/mission-statement', icon: MegaphoneIcon },
    // { name: 'Contact', description: "Venez à nous ou contactez-nous.", href: '/explore/contact', icon: DevicePhoneMobileIcon },
    { name: 'Blog', description: "Retrouvez toutes nos publications.", href: '/explore/blog', icon: NewspaperIcon },
]
...
export default function Toolbar() {
    ...
    const { events, ministries, cms: { global: { app_name }, frontend: { header: { menu } } } } = content!

    const eventsItems = events.map(({ title, description, link, photo }) => ({ name: title, href: link, description, photo }))
    const ministriesItems = ministries.map(({ name, link, description, photo }) => ({ name, href: link, description, photo }))
                ...
                <div className="container">
                    <div className="flex items-center py-[12px] md:py-[10px]">
                        ...
                        <div className='ml-auto flex items-center space-x-3 md:space-x-5'>
                            ...
                            <div className='flex items-center md:border-l md:border-secondary-200 md:ml-6 md:pl-6 dark:md:border-secondary-200/20'>
                                <button type="button" id="headlessui-listbox-button-10" className='block h-10 p-2 -m-2' aria-haspopup="true" aria-expanded="false" data-headlessui-state="" aria-labelledby="headlessui-listbox-label-9 headlessui-listbox-button-10" onClick={toggleDark}>
