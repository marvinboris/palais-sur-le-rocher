...
const renderItem = (item: Item, close: () => void) => <Link key={item.name} href={item.href} onClick={close} className="-m-3 flex items-start rounded-lg p-3 hover:bg-secondary-50 dark:hover:bg-secondary-800">
    {item.icon && <item.icon className="h-6 w-6 flex-shrink-0 text-primary-600" aria-hidden="true" />}
    {item.photo && <div className='h-6 w-6 flex-shrink-0'><Image width={1920} height={1920} src={item.photo} alt={item.name} className="image-cover rounded" /></div>}
    <div className="ml-4">
...
const renderAction = (item: Action) => <div key={item.name} className="flow-root">
    <Link href={item.href} className="-m-3 flex items-center rounded-md p-3 text-base font-medium text-secondary-900 hover:bg-secondary-100 dark:text-secondary-50 dark:hover:bg-secondary-700">
...
const renderListItem = (item: ListItem) => (
    <li key={item.id} className="truncate text-base">
        <Link href={item.href} className="font-medium text-secondary-900 hover:text-secondary-700">
            {item.name}
        </Link>
    </li>
)
