...
export default function NavItem({ href, icon: Icon, exact, children }: NavItemProps) {
    ...
    return href.startsWith('#') ? <a href={href} className={classNames(active ? 'text-primary-600' : 'text-secondary-700 hover:text-primary-600', "truncate font-semibold text-sm leading-6 dark:text-secondary-200 dark:hover:text-primary-600 inline-flex items-center transition-all duration-200")}>
        {content}
    </a> : <Link href={href} className={classNames(active ? 'text-primary-600' : 'text-secondary-700 hover:text-primary-600', "truncate font-semibold text-sm leading-6 dark:text-secondary-200 dark:hover:text-primary-600 inline-flex items-center transition-all duration-200")}>
        {content}
    </Link>
}