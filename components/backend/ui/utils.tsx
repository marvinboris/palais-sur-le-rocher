...
const Redirection = ({
  ...
  if (role === "user") {
    const feature = (data as ApiAccountUserType).role.features.find(
      (f) => f.prefix === resource.split("_").join("-")
    );
    if (!(feature && feature.access.includes(props.edit ? "u" : "c")))
...
export const add = {
  ...
  lifecycle: {
    ...
    componentDidUpdate:
      (resource: ResourceType, singular: string) =>
      (
        props: PropsType,
        ...
        if (
          props.backend.message &&
          props.backend.message.type === "success" &&
          !props.edit
        ) {
          if (state.add) resetState();
          else {
            const pathname = `/${props.auth.role}/${resource
              .split("_")
              .join("-")}`;
            props.history
              ?.push(
        ...
        if (
          (props.backend.data && props.backend.data[singular]) ||
          (props.edit &&
            props.backend.message &&
            props.backend.message.type === "success")
        ) {
        ...
        content = (
          <div className="px-[33px] pt-[29px] pb-[54px] md:px-[42px] md:pt-[47px]">
            {!message &&
              props.history &&
              props.history.query &&
              props.history.query.type && (
                <Alert
                  className="mb-4"
                  color={
                    props.history.query.type as
                      | "info"
                      | "danger"
                      | "success"
                      | "warning"
                ...
                >
                  {props.history.query.content as string}
                </Alert>
              )}
            {message && (
              <Alert className="mb-4" color={message.type}>
