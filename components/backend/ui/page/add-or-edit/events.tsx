...
export default function ManageAddOrEditServices({ edit }: Props) {
    ...
    const { cms: { backend: { components: { form: { active, inactive } }, pages: { events: { form } } } } } = content!
    ...
    return <ManagerAddOrEdit icon={BellIcon} edit={edit} resource='events' singular='event' initialState={initialState} state={state} setState={setState} staticChild={<>
        ...
        <div className='grid md:grid-cols-3 gap-4'>
            <div className="md:col-span-2">
                <div className="flex-1 grid gap-y-2 gap-x-4 grid-cols-1 md:grid-cols-2 overflow-auto">
                    <Input icon={BellIcon} onChange={inputChangeHandler} value={state.title as string} name="title" required validation={{ required: true }} label={form.title} />
