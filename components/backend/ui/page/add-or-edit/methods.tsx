...
export default function ManageAddOrEditMethods({ edit }: Props) {
    ...
    const { cms: { backend: { components: { form: { active, inactive } }, pages: { methods: { form } } } } } = content!
    ...
    return <ManagerAddOrEdit icon={ShoppingBagIcon} edit={edit} resource='methods' singular='method' initialState={initialState} state={state} setState={setState} staticChild={<>
        ...
        <div className='grid md:grid-cols-3'>
            ...
            <div className='md:flex items-center justify-center'>
                <div onClick={() => fileUpload('logo')} className="aspect-[5/2] md:w-40 md:aspect-square cursor-pointer mt-[14px] md:mt-0 rounded-[15px] md:rounded-3xl relative flex flex-col items-center justify-center overflow-hidden text-white">
                    {state.logo && <Image width={1920} height={1920} src={state.logo as string} alt="User profile pic" className="absolute z-0 inset-0 image-cover" />}
