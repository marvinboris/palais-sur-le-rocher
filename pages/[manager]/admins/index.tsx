...
const ManagerAdminsPage: NextPageWithLayout = () => {
    ...
    const { cms: { global: { app_name }, backend: { components: { list: { action, see } }, pages: { admins: { form, index, title } } } } } = content!

    const [isMounted, setIsMounted] = useState(false)
    const [params] = useState({ role: role!, resource: 'admins' })

    useEffect(() => {
        if (status === Status.IDLE && !(backend && backend.admins)) {
            dispatch(get(params))
            setIsMounted(true)
        }
    ...
    const data = (backend && backend.admins ? (backend.admins as AdminInterface[]) : []).map(admin => {
        return updateObject(admin, {
            ...
            action: <Action props={props} resource='admins' item={admin} />,
        });
    });

    return <main className='flex-1 flex flex-col'>
        <Head link={`/${role}/admins`} title={`${index} | ${app_name}`} description={`${app_name} : ${index}`} />

        <PageTitle icon={UserGroupIcon} title={title} subtitle={index} />

        <utility.index.lifecycle.render icon={UserGroupIcon} props={props} isMounted={isMounted} resource='admins' data={data!} fields={[
