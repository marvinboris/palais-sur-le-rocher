...
const params = {
  ...
  description:
    "Ecoutez les messages de notre Seigneur le long de nos différentes rencontres.",
};
...
const LessonsPage = ({ lessons }: LessonsPageProps) => {
  ...
  const onClick = (lesson: LessonType) => {
    if (currentLesson !== null && lesson.audio === currentLesson.audio)
      setActive(!active);
    else setLesson(lesson);
  };

  const renderLesson = (lesson: LessonType, index: number) => (
    <LessonBlock
      key={`lesson-${lesson.link}-${index}`}
      {...lesson}
      current={
        currentLesson !== null && lesson.audio === currentLesson.audio && active
      }
      ...
      <main id="lessons" className="page-main">
        <header className="relative">
          ...
          <div className="container relative z-10 text-secondary-50">
            <p className="text-xs uppercase sm:text-sm">Liste de lecture</p>
            ...
            <p className="mt-8 text-xs sm:text-sm">
              {lessons.length} enseignements,{" "}
              <span className="font-semibold">
                <DurationBlock urls={lessonsUrls} />
              </span>
            </p>
          </div>
        </header>
