...
const LessonPage = ({ lessons }: LessonPageProps) => {
  ...
  const onClick = () => {
    if (currentLesson !== null && audio === currentLesson.audio)
      setActive(!active);
        ...
        <header className="relative">
          <div className="bg-img">
            <Image
              width={1920}
              height={1920}
              src="/images/aaron-burden-cmIqkMPfpMQ-unsplash.jpg"
              alt="Enseignements"
              className="image-cover"
            />
          </div>

          <div className="container relative z-10 text-secondary-50 md:flex md:space-x-6">
            <div className="flex justify-end">
              <button
                type="button"
                onClick={onClick}
                className="group relative flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-full bg-secondary-50 focus:outline-none focus:ring-2 focus:ring-secondary-50 focus:ring-offset-2 hover:bg-secondary-200"
                aria-label="Play"
              >
                ...
                {!active ? (
                  <svg
                    aria-hidden="true"
                    viewBox="0 0 36 36"
                    className="h-7 w-7 fill-secondary-700 group-active:fill-secondary-900"
                  >
                  ...
                  <svg
                    aria-hidden="true"
                    viewBox="0 0 22 28"
                    className="h-5 w-5 fill-secondary-700 group-active:fill-secondary-900"
                  >
                    <path
                      fillRule="evenodd"
                      clipRule="evenodd"
            ...
            <div>
              <p className="text-xs uppercase sm:text-sm">Enseignement</p>
...
export async function getServerSideProps() {
  ...
  return {
    props: {
      lessons: JSON.parse(
        JSON.stringify(lessons.map((lesson) => lesson.toObject()))
      ),
    },
  };
}
